...
class NeuralNetwork(ABC):
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        dLdX = self.loss.backward(target, out)
    ...
    def train(self, dataset: Dataset, epochs: int) -> None:
        ...
        for i in range(epochs):
            ...
            if self.name == "higgs_model1" and round(1 - validation_error, 4) >= .7:
                print("\n\nEPOCHS REQUIRED TO GET VAL_ACCURACY 70%:", i)
                self.save_parameters(i)

            print("Time taken this epoch:", datetime.datetime.now() - begin_time)
